muvie_box={
  ...
  let arr=muvie_box.Search
  let result=arr.map(el=>{
    return`
    <div class="col-3 bg-danger m-5">
        <img src="${el.Poster}" alt="" class="img1">
        <h1 class="text-center" >${el.Title}</h1>
        <H3 class="text-center">${el.Year}</H3>
        <p class="text-center">${el.Type}</p>
        <p class="text-center">${el.imdbID}</p>
    </div>
    `
  })
